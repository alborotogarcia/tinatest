...
export const useCreateMDBlogPage = (allBlogs = []) => {
  const router = useRouter()
  const cms = useCMS()
  usePlugins([
    {
      __type: 'content-creator',
      name: 'Make a new markdown blog',
      // @ts-ignore
      fields: [
        {
          name: 'title',
      ...
      onSubmit: async (frontMatter) => {
        ...
        const fileRelativePath = `content/mdblog/${fileName}.md`
        return await github
          .commit(
            fileRelativePath,
            ...
            'Add new markdown blog page'
          )
          .then((response) => {
            setCachedFormData(fileRelativePath, {
              sha: response.content.sha
            })
            setTimeout(() => router.push(`mdblog/${fileName}`), 1500)
          })
          .catch((e) => {
            return { [FORM_ERROR]: e }
          })
